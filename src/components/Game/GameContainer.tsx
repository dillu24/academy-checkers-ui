...
export default class GameContainer extends Component<IGameContainerProps,
  IGameContainerState> {
  ...
  public async loadGame(): Promise<void> {
    const game: IGameInfo | undefined = await (await this.getStargateClient()).getGuiGame(
      this.props.index)
    if (!game) {
  ...
  public async makeMove(): Promise<void> {
    ...
    const positions: Position[] = keys.map(
      (k: string): Position => k.split(",").map(Number) as Position
    );

    const client = await this.getStargateClient()
    const canPlay = await client.canPlayGuiMove(
      this.props.index,
      this.state.board.current_player,
      positions
    )
    if (!canPlay.possible) {
      const error = `Cannot make this move ${canPlay.reason}`
      alert(error)
